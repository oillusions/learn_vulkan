    ...
    public:
        class FromFunctor : detail::ErrorRecord {
            ...
            {};
              ...
              template <typename E> 
              Error operator()(const E &e) noexcept;
        };

        class UnwrapFunctor : protected detail::ErrorRecord {
            public:
                UnwrapFunctor(
                    std::string message,
                    const std::source_location location
                ...
                T operator () (
                    ...
                    if (!expected) {
                        throw std::move(expected.error()).forward(std::move(*this));
                    }
        ...
        requires std::invocable<FromFunctor, std::expected<T, Error>>
        friend auto operator | (
            std::expected<T, Error>&& expected, 
            Functor&& func
        )
            noexcept(noexcept(std::forward<Functor>(func)(std::move(expected))))
            -> decltype(std::forward<Functor>(func)(std::move(expected))) {
            return std::forward<Functor>(func)(std::move(expected));
        }
